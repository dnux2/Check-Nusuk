    ...
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-lg">System status across all sectors.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        <StatCard delay={0.1} title="Total Pilgrims Tracked" value={totalPilgrims.toLocaleString()} icon={<Users className="w-6 h-6" />} trend="+2.4%" trendUp={true} />
        <StatCard delay={0.2} title="Active Emergencies" value={activeEmergencies} icon={<AlertTriangle className="w-6 h-6" />} trend="-1" trendUp={true} />
        <StatCard delay={0.3} title="Security Alerts" value={activeAlerts} icon={<ShieldAlert className="w-6 h-6" />} trend="+4" trendUp={false} />
        <StatCard delay={0.4} title="Avg Crowd Density" value="68%" icon={<Activity className="w-6 h-6" />} trend="-5%" trendUp={true} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          <h2 className="text-xl font-display font-bold mb-6">Live Sector Map</h2>
          <div className="flex-1 min-h-[400px]">
            <MockMap activePoints={15} />
          </div>
        </motion.div>

        {/* Right Column Charts/Feeds */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
          className="bg-card rounded-2xl border border-border/50 shadow-sm p-6 flex flex-col"
        >
          <h2 className="text-xl font-display font-bold mb-6">Crowd Density Trend</h2>
          ...
          <h2 className="text-xl font-display font-bold mt-8 mb-4">Recent Alerts</h2>
          <div className="space-y-3 flex-1 overflow-auto pr-2">
            {(alerts || Array.from({length: 4})).slice(0, 4).map((alert: any, i) => (
              <div key={alert?.id || i} className="p-4 rounded-xl bg-background border border-border hover:border-primary/30 transition-colors">
                ...
                <p className="text-sm text-muted-foreground truncate">{alert?.message || 'Camera 4A detected unpermitted access.'}</p>
              </div>
            ))}
          </div>
        </motion.div>
